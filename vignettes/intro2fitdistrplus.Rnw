...
Fitting distributions to data is a very common task in statistics. It consists in choosing
...
the process may be developed using R \cite{Ricci05}. Our first objective by developping
...
% POUR JSS citation papier actuar
% POUR JSS faudra replacer notre package par rapport au package distrMod qui manipule
% des distributions sous forme d objets de type S4 et propose aussi diverses methodes d ajustement
% cf. package et article dans JSS (Kohl 2010)
% CHRISTOPHE, EST-CE QUE TU CONNAIS CE PACKAGE ET TE SENS d'ECRIRE CA??????
% Ca a l air assez lourd ‡ utiliser vu comme cela, et il ne me semble pas que cela permette de
% prendre en compte des donnÈes censurees
% ON PEUT AUSSI METTRE CE PARAGRAPHE EN DISCUSSION, A VOIR 
...
This manuscript reviews the various features of version 0.4-4 of \texttt{fitdistrplus}. The package
...
The ``Toxocara'' data set corresponds to the observation of a discrete variable, 
...
The ``smoked fish'' data set corresponds to the observation of a continuous censored variable, 
...
\begin{figure}[htb!]
  \centering
  %%% R code
<<figtoxocaraplot, echo=FALSE, fig=TRUE, width=10, height=5, eps=FALSE>>=
plotdist(toxocara$number,discrete = TRUE)
@
...
unbiased estimations of the three last statistics are provided but the argument \texttt{method}
may be used to obtain  them without correction for bias. 
Formulas for unbiased skewness  and kurtosis :
  \begin{equation}
    \label{skewness}
    skewness=\frac{\sqrt{n(n-1)}}{n-2}\times\frac{m_{3}}{m_{2}^{\frac{3}{2}}}
  \end{equation}

  \begin{equation}
    \label{kurtosis}
    kurtosis=\frac{n-1}{(n-2)(n-3)}((n+1) \times \frac{m_{4}}{m_{2}^{2}}-3(n-1)) + 3
  \end{equation}
with $m_{2}$, $m_{3}$, $m_{4}$ moments defined by
...
skewness-kurtosis plot. Below is a call to function \texttt{descdist} to describe the distribution 
...
\begin{figure}[htb]
  \centering
 %%% R code
<<descgroundbeefplot, echo=FALSE, fig=TRUE, width=5, height=5, eps=FALSE>>=
descdist(groundbeef$serving, boot=1000)
@
...
For discrete variables, such as the number of \emph{Toxocara cati} parasites from 
the ``Toxacara'' data set,  skewness and kurtosis values or set of values of Poisson and negative binomial
distributions are represented in the skewness-kurtosis plot 
...
\section{Fit of a distribution by maximum likelihood estimation}
\label{FIT}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Below is a call to function \texttt{fitdist} to fit a Weibull distribution 
...
four goodness-of-fit plots are provided : a draw of pdf curve and histogram together (density plot), an cdf plot
...
by a call to Function \texttt{ppoints} from the \texttt{stats} package with its default arguments.
...
As an example, let us look at the plot of the previous fit of a Weibull distribution to the``groundbeef'' data set
(Figure~\ref{plotcontfit}). The fit is not perfect,
especially in the center of the distribution, but seems correct while looking at the tails.
%%% R code
...
\begin{figure}[htb]
  \centering
  %%% R code
<<fitgroundbeefweibullplot, echo=FALSE, fig=TRUE, width=5, height=5, eps=FALSE>>=
plot(fw)
@
  \caption{Plot of the fit of a continuous distribution (a Weibull
  distribution fitted to serving sizes from the ``ground beef'' data set)}
  \label{plotcontfit}
\end{figure}

For continuous distributions, Function \texttt{denscomp}, \texttt{cdfcomp},
\texttt{qqcomp} and \texttt{ppcomp}, enable the visual comparison
of the empirical and various theoretical distributions fitted
on a same data set, using one of the four plots
provided by \texttt{plotdist}. These functions must be called with a first argument 
corresponding to a list of objects of class \texttt{fitdist}, and optionaly
...
Weibull, lognormal and gamma distributions to ``groundbeef'' data set with a density plot and a cdf plot
(Figure~\ref{compdens} and ~\ref{compcdf}).
%%% R code
...
For discrete distributions, the plot of an object of class \texttt{fitdist} simply provides
...
%  fitted to numbers of \emph{Toxocara cati} parasites from the ``Toxocara'' data set)}
%  \label{plotdiscfit}
%\end{figure}
...
\label{GOF measures}
%%%%%%%%%%
The purpose of goodness-of-fit statistics is to measure the distance beetween
the cumulative distribution function $F$ defined from the fitted parametric distribution
with the empirical distribution function $F_{n}$ defined from the data.
When fitting continuous distributions, three classical goodness-of-fit statistics, 
Cramer-von Mises, Kolmogorov-Smirnov and Anderson-Darling statistics, may be computed using 
the function \texttt{gofstat}
as defined by Stephens \cite{Stephens86} (Table \ref{tabKSCvMAD}).
...
\begin{table}[t]
  \begin{center}
    ...
    \hline
      ...
      $max(D^{+},D^{-})$ with\\
      (KS)&& $D^{+}=max_{i}(\frac{i}{n} - F(x_{i}));D^{-}=max_{i}(F(x_{i})-\frac{i-1}{n})$ \\
    \hline
      Cramer-von Mises  & n $\int_{-\infty}^{\infty}(F_{n}(x) - F(x))^2 dx$ &
        $\frac{1}{12n} + \sum_{i}(F(x_{i})-\frac{2i-1}{2n})^{2}$\\
      (CvM)&&\\
% cvm <- 1/(12*n) + sum( ( theop - (2 * seq(1:n) - 1)/(2 * n) )^2 )
    \hline
      Anderson-Darling  & n $\int_{-\infty}^{\infty}(F_{n}(x) - F(x))^2 \psi(x) dx$
      & $-n -\frac{1}{n}\sum_{i}((2i-1)(ln(F(x_{i}))+ln(1-F(x_{n+1-i}))))$ \\
      (AD) &  with $\psi(x) = (F(x) \times (1 - F(x)))^{-1}$  &\\
...
continuous distributions (see the reference 
manual~\cite{fitdistrplus} for examples) for examples).
...
The results of the tests are not printed, unless the argument \texttt{print.test}
is fixed to \texttt{TRUE}. We chose not to print their results by default, as 
...
the empirical distribution may be plotted using the \texttt{plotdistcens}function.
By default this function uses the EM approach of Turnbull \cite{Turnbull74} to compute the overall
empirical cdf curve with confidence intervals, by calls to \texttt{survfit}
and \texttt{plot.survfit} functions from the \texttt{survival} package.
Let us see such a plot for ``smokedfish'' data set after classical transformation
...
\begin{figure}[htb!]
  \centering
  %%% R code
<<plotsmokedfishplot, echo=FALSE, fig=TRUE, width=4, height=4, eps=FALSE>>=
plotdistcens(log10C)
@
...
the \texttt{fitdistcens}function. This function estimates distribution parameters $\theta$
...
The \texttt{plotdistcens} function can help to find correct initial values for 
...
in Section~\ref{censored graph}. The \texttt{cdfcompcens} function can be used to compare the fit of various
distributions to the same censored data set. Its call is similar to the one \texttt{cdfcomp}. Below is
an example of comparison of two fitted distribution to ``smokedfish'' data set (Figure~\ref{cdfcompcens}).
%%% R code
...
\label{MGE}
%%%%%%%%%%
Maximum likelihood is only the default estimation method proposed by 
...
the parameters $\theta$ at the values that makes the first theoretical moments of the parametric 
...
This method called moment matching estimation can be performed fixing the argument \texttt{method}
...
sum of squared differences between observed and theoretical moments (see the \texttt{fitdistrplus} reference
...
\label{QME}
%%%%%%%%%%
Fitting of a parametric distribution may also be done by estimating
the parameters $\theta$ at the values that makes theoretical quantiles of the parametric 
distribution for specified probabilities equal to the empirical quantiles (Equation~\ref{quantiles}).
  \begin{equation}
    \label{quantiles}
...
Quantile matching can be performed by fixing the argument \texttt{method}
...
\texttt{quantile} function of the \texttt{stats} package using the \texttt{type} 
argument  equal to 7 by default, but the type of quantile
can be easily changed using the \texttt{qty} argument in the call to the \texttt{qme} function.   
...
the \texttt{optim} function of the \texttt{stats} package  is used by default with the 
...
be interesting to change some options of the \texttt{optim} function or to use another optimization
...
Here are examples of fits of a gamma distribution to ``ground beef'' data set with various options
of \texttt{optim}.
...
% POUR JSS CA DEVIENT HYPER TECHNIQUE, on enleve? on revoit aux exemples de fitdist? on met en annexe?
...
%%% R code
<<optimmethod.customgenoud.fitdist, echo=TRUE, eval=FALSE>>=
...
\section{Uncertainty in parameter estimates}
\label{Uncertainty}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
bootstrap using the \texttt{boodist} function for non censored data and
by nonparametric bootstrap using \texttt{boodistcens} function for censored data. 
These functions return the boostrapped values of parameters in a 
...
The plot of an object of class \texttt{bootdist}  or \texttt{bootdistcens}
 consists in a scatterplot or a matrix of scatterplots
of the bootstrapped values of parameters providing
a representation of the joint uncertainty distribution of the fitted parameters (Figure~\ref{bootstrap}).


Below is an example of the use of the \texttt{bootdist} function with the previous of the Weibull distribution to 
``groundbeef`` data set. 
%%% R code
...
\begin{figure}[htb!]
  \centering
  %%% R code
<<fitgroundbeefweibullbootplot, echo=FALSE, fig=TRUE, width=4, height=4, eps=FALSE>>=
plot(bw)
@
...
serving sizes from the ``ground beef'' data set)}
  \label{bootstrap}
\end{figure}
...
% PERHAPS ADD an example with the Burr distribution characterized by three parameters
% to show high correlations between parameters


\subsection{Use of bootstrap samples}
%\label{MC2D}
%%%%%%%%%%
%TO BE DONE
...
simulation framework, especially within the package \texttt{mc2d} (\cite{mc2d}). One could refer to Pouillot \emph{et al.}
(\cite{Pouillot10}) for an introduction to the use of \texttt{mc2d} 
and \texttt{fitdistrplus} packages in the context of quantitative 
risk assessment.

Bootstrap can also be used to calculate confidence intervals on quantiles of the fitted distribution.
For this purpose, a generic \texttt{quantile} function is provided for class \texttt{fitdist} for non censored data and \texttt{fitdistcens} for censored data. 
They must be called with a first argument corresponding
to an object of class \texttt{fitdist} or \texttt{fitdistcens}, and as a second argument the vector of probabilities
...
These two functions are internally calling to \texttt{bootdist} or \texttt{bootdistcens} and give
the complete results of these calls as a part of their output, in \texttt{resbootdist} and \texttt{resbootdistcens}.
The \texttt{quantile} function is of great interest in the ecotoxicology
...
Below is an example of use of \texttt{quantile} to estimate HCx values for different x-values, with
$95\%$ bootstrap confidence intervals, using the maximum likelihood
 fit of a lognormal distribution on the ``endosulfan'' data set.
 %%% R code
