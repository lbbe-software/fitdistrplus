...
to help the fit of a distribution to continuous data, we will use
a data set named ``ground beef'' and included in the package.
...
children under 5 years old. It was used in a quantitative risk assessment published
...
Formulas for unbiased skewness  and kurtosis are :
  \begin{equation}
    \label{skewness}
    unbiased~skewness=\frac{\sqrt{n(n-1)}}{n-2}\times\frac{m_{3}}{m_{2}^{\frac{3}{2}}}
  \end{equation}

  \begin{equation}
    \label{kurtosis}
    unbiased~kurtosis=\frac{n-1}{(n-2)(n-3)}((n+1) \times \frac{m_{4}}{m_{2}^{2}}-3(n-1)) + 3
  \end{equation}
...
%\footnote{The 
% bootstrap is a statistical procedure providing random samples of statistical estimates.}
...
ADD THE TWO OTHER GRAPHS, AND COMMENT THEM, ESPECIALLY THE QQPLOT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
\label{GOF tests}
%%%%%%%%%%
TO BE REMOVED AT LEAST IN THE JSS VERSION
...
Function \texttt{descdist} can be used, but with the argument \texttt{discrete}
fixed to \texttt{TRUE}. This function will especially 
...
As an example, 
using the ``toxocara'' data set, Poisson and
...
The ``smoked fish'' data set, included in the package,
