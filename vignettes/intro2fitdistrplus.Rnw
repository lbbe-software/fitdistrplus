...
fCG <- fitdist(groundbeef$serving, "gamma", optim.method="CG", control=list(maxit=5000))
@
...
fgenoud <- fitdist(groundbeef$serving, "gamma", custom.optim=mygenoud, nvars=2,    
...
\begin{figure}[htb!]
  ...
  \subfloat{
  %%% R code
<<fitgroundbeefweibullbootplot, echo=FALSE, fig=TRUE, width=3, height=3, eps=FALSE>>=
plot(bw)
@
}
\subfloat{
  %%% R code
<<fitdanishburrbootplot, echo=FALSE, fig=TRUE, width=3, height=3, eps=FALSE>>=
plot(bdan)
@
}
