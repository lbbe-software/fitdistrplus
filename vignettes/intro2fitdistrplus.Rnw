...
In this paper, we present our package \pkg{fitdistrplus} for the statistical software \proglang{R} of \cite{R12}.

Function \code{fitdistr} in the \proglang{R} package \pkg{MASS} by \cite{MASS} 
...
Papers citing \pkg{fitdistrplus} are \cite{busschaertetal10,brooksetal11,aktassjostrand11,eling12,croucheretal12,commeauetal12,bagariaetal12,anandetal12,eikhermann12}
...
First of all, an empirical distribution may be plotted using classical \proglang{R} function
or using Function \code{plotdist} which provides plots
...
of the estimated values of kurtosis and skewness from data, the data set may be boostraped\footnote{The bootstrap is a statistical procedure providing random samples of statistical estimates.} by 
fixing the argument \code{boot} to an integer above 10. Values of skewness and kurtosis 
...
Function \code{fitdist}. By default, distribution parameters $\theta$ are estimated by maximizing the
...
\label{GOF plots}
%%%%%%%%%%
The plot of an object of class \code{"fitdist"} provides two types of results depending of the nature 
...
For continuous distributions, Function \code{denscomp}, \code{cdfcomp},
...
\code{fitdist}, but other methods may be used to estimate parameters for non-censored data.
...
to ``mge'' in the call to \code{fitdist} and to specify the argument \texttt{gof}
...
For other distributions, Function \code{fitdist} carries out 
...
to ``qme'' in the call to fitdist and adding an argument \code{probs}
...
function than \code{optim} to maximize the likelihood.
...
It is assumed that \code{custom.optim} should carry out a MINIMIZATION. Finally, it should return
...
Below is an example of code written to customize \code{genoud} function from \pkg{rgenoud} package.
%%% R code
...
``ground beef'' data set. Note that in this example various arguments are also passed from \code{fitdist} 
to \code{genoud} :
