...
\item $\beta_k = \frac{ g_k^T (g_k-g_{k-1} )}{g_{k-1}^T g_{k-1}}$: Polak-Ribière update.
\end{itemize}
...
fitbench <- function(data, distr, method, grlnL, ...) 
{
  ...
  {
    nograd$time <- system.time(nograd <- mledist(x, dist=distr, optim.method=meth, control=ctr, ...))[3]
    grad$time <- system.time(grad <- mledist(x, dist=distr, gr=grlnL, optim.method=meth, control=ctr, ...))[3]
    reslist <- c(reslist, list(nograd), list(grad))
  }
  for(type in 2:3) #CG with PR or BS updates
  {
    nograd$time <- system.time(nograd <- mledist(x, dist=distr, optim.method="CG", control=c(ctr, type=type), ...))[3] 
    grad$time <- system.time(grad <- mledist(x, dist=distr, gr=grlnL, optim.method="CG", control=c(ctr, type=type), ...))[3] 
    reslist <- c(reslist, list(nograd), list(grad)) 
  }
  fullname <- c("BFGS", "NM", paste("CG", c("FR", "PR", "BS")))
  fullname <- as.vector(sapply(fullname, function(x) paste0(x, c("", " grad."))))
  ...
  resCGFR <- sapply(reslist[grep("CG FR", names(reslist))], getval)
  resCGPR <- sapply(reslist[grep("CG PR", names(reslist))], getval)
  resCGBS <- sapply(reslist[grep("CG BS", names(reslist))], getval)  
  resBFGS <- sapply(reslist[grep("BFGS", names(reslist))], getval)  
  rownames(resNM) <- rownames(resCGFR) <- rownames(resCGPR) <- rownames(resCGBS) <- rownames(resBFGS) <- c("fitted shape1", "fitted shape2", "fitted loglik", "func. eval. nb.", "grad. eval. nb.", "time (sec)")
...
```{r}
fitbench(x, "beta", "mle", grlnL) 

reslist <- NULL
for(meth in c("BFGS", "Nelder", "CG")) #CG with FR update
{
  nograd$time <- system.time(nograd <- mledist(x, dist="beta", optim.method=meth, control=ctr))[3]
  grad$time <- system.time(grad <- mledist(x, dist="beta", gr=grlnL, optim.method=meth, control=ctr))[3]
  reslist <- c(reslist, list(nograd), list(grad))
}
for(type in 2:3) #CG with PR or BS updates
{
  nograd$time <- system.time(nograd <- mledist(x, dist="beta", optim.method="CG", control=c(ctr, type=type)))[3] 
  grad$time <- system.time(grad <- mledist(x, dist="beta", gr=grlnL, optim.method="CG", control=c(ctr, type=type)))[3] 
  reslist <- c(reslist, list(nograd), list(grad)) 
}
fullname <- c("BFGS", "NM", paste("CG", c("FR", "PR", "BS")))
fullname <- as.vector(sapply(fullname, function(x) paste0(x, c("", " grad."))))
names(reslist) <- fullname
#lapply(reslist, names)
```
...
for(meth in c("BFGS", "Nelder", "CG")) #CG with FR update
{
  nograd$time <- system.time(nograd <- mledist(x, dist="beta", custom.optim=constrOptim2,
      ui = diag(2), ci = c(0, 0), optim.method=meth, control=ctr))[3]
  grad$time <- system.time(grad <- mledist(x, dist="beta", gr=grlnL, custom.optim=constrOptim2,
      ui = diag(2), ci = c(0, 0),  optim.method=meth, control=ctr))[3]
  reslist <- c(reslist, list(nograd), list(grad))
}
for(type in 2:3) #CG with PR or BS updates
{
  nograd$time <- system.time(nograd <- mledist(x, dist="beta", custom.optim=constrOptim2,
      ui = diag(2), ci = c(0, 0), optim.method="CG", control=c(ctr, type=type)))[3] 
  grad$time <- system.time(grad <- mledist(x, dist="beta", gr=grlnL, custom.optim=constrOptim2,
      ui = diag(2), ci = c(0, 0), optim.method="CG", control=c(ctr, type=type)))[3] 
  reslist <- c(reslist, list(nograd), list(grad)) 
}
names(reslist)[(length(fullname)+1):length(reslist)] <- paste("constr.", fullname)
```
...
for(meth in c("BFGS", "Nelder", "CG")) #CG with FR update
{
  nograd$time <- system.time(nograd <- mledist(x, dist="beta2", optim.method="BFGS", control=ctr, start=startarg))[3]
  grad$time <- system.time(grad <- mledist(x, dist="beta2", gr=grlnL, optim.method=meth, control=ctr, start=startarg))[3]
  nograd$estimate <- exp(nograd$estimate)
  grad$estimate <- exp(grad$estimate)
  reslist <- c(reslist, list(nograd), list(grad))
}
for(type in 2:3) #CG with PR or BS updates
{
  nograd$time <- system.time(nograd <- mledist(x, dist="beta2", optim.method="CG", control=c(ctr, type=type), start=startarg))[3] 
  grad$time <- system.time(grad <- mledist(x, dist="beta2", gr=grlnL, optim.method="CG", control=c(ctr, type=type), start=startarg))[3] 
  nograd$estimate <- exp(nograd$estimate)
  grad$estimate <- exp(grad$estimate)
  reslist <- c(reslist, list(nograd), list(grad)) 
}
names(reslist)[(2*length(fullname)+1):length(reslist)] <- paste("exp.", fullname)
```
...
resgenoud$time <- system.time(resgenoud <- mledist(x, "beta", start=startarg, custom.optim= mygenoud, 
        nvars=2, Domains=cbind(c(0,0), c(100, 100)), boundary.enforcement=1, 
...
```{r}
cmin <- min(sapply(reslist, function(x) x$counts[1]))
names(reslist)[sapply(reslist, function(x) x$counts[1]) == cmin]
```
That is BFGS, Nelder-Mead and CG with Fletcher-Reeves update methods, all with
the exponential transformation of the parameters.
Since the exponential function is differentiable, the asymptotic properties are
still preserved (Delta method) but for finite-sample this may produce a small bias.
The second-best method is the BFGS with neither gradient, neither constaint, nor transform
```{r}
cmin <- sort(unique(sapply(reslist, function(x) x$counts[1])))[2]
names(reslist)[sapply(reslist, function(x) x$counts[1]) == cmin]
```
The difference in terms of fitted values are neglible.
```{r}
resBFGS[, c("BFGS", "exp. BFGS")]
```
...
library(actuar)
#(2) Burr distribution
y <- rburr(n, 2, 3, scale=10)
hist(y, prob=TRUE)
lines(density(y), col="red")
curve(dburr(x, 2, 3, scale=10), col="green", add=TRUE)
...
```{r}
initpar <- list(shape1=10, shape2=10, scale=1)
reslist <- NULL
for(meth in c("BFGS", "Nelder", "CG")) #CG with FR update
{
  nograd$time <- system.time(nograd <- mledist(y, dist="burr", optim.method=meth, start =initpar, control=ctr))[3]
  grad$time <- system.time(grad <- mledist(y, dist="burr", optim.method=meth, start =initpar, control=ctr, gr=grburr))[3]
  reslist <- c(reslist, list(nograd), list(grad))
}
for(type in 2:3) #CG with PR or BS updates
{
  nograd$time <- system.time(nograd <- mledist(y, dist="burr", optim.method="CG", start =initpar, control=c(ctr, type=type)))[3]
  grad$time <- system.time(grad <- mledist(y, dist="burr", optim.method="CG", start =initpar, control=c(ctr, type=type), gr=grburr))[3] 
  reslist <- c(reslist, list(nograd), list(grad)) 
}
fullname <- c("BFGS", "NM", paste("CG", c("FR", "PR", "BS")))
fullname <- as.vector(sapply(fullname, function(x) paste0(x, c("", " grad."))))
...
```
```{r}
sapply(reslist[grep("NM", names(reslist))], getval)
sapply(reslist[grep("CG", names(reslist))], getval)
sapply(reslist[grep("BFGS", names(reslist))], getval)
```
