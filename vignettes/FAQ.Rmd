...
There is no density (w.r.t. the Lebesgues meausre) since
...
```
The density function with respect to the measure $m(x)= \delta_l(x)+\delta_u(x)+\lambda(x)$ is
$$
f_Y(y) = 
\left\{\begin{array}{ll}
F_X(l) & \text{if } y=l \\
...
Hence, the  likelihood can be factorized as 
$$
...
$$
Since $\forall i, l\leq y_i\leq u\Leftrightarrow l\leq \min_i y_i\leq \max_i y_i\leq u$,
...
The maximum of $L$ is reached at $l=\min_i y_i$ and $u=\max_i y_i$,
i.e.~the MLE is $\hat l=\min_i Y_i$ and $\hat u=\max_i Y_i$,
The MLE of $\theta$ is obtained by maximizing the log-likelihood $\log(L)$
with $u=\hat u$ and $l=\hat l$.
```{r, message=FALSE, fig.height=4, fig.width=4}
n <- 1000
x <- pmax(pmin(rexp(n), 3), .5)
f1 <- fitdist(x, "texp", method="mle", start=list(rate=1/mean(x), low=.1, upp=1e1), lower=0)
f2 <- fitdist(x, "texp", method="mle", start=list(rate=1/mean(x)), fix.arg=list(low=min(x), upp=max(x)))
f3 <- fitdist(x[x> min(x) & x < max(x)], "exp", method="mle")

cdfcomp(list(f1, f2), do.points = FALSE, addlegend=FALSE)
curve(ptexp(x, f3$estimate, min(x), max(x)), add=TRUE, col="blue", lty=3)
curve(ptexp(x, 1, .5, 3), add=TRUE, col="darkorange", lty=4)
legend("bottomright", lty=1:4, col=c("red", "green", "blue", "darkorange"), 
        leg=c("full MLE", "MLE fix arg", "MLE on subdata", "true cdf"))
```
...
statistics in case of a fitted distribution.
