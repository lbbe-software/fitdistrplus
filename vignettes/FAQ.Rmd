...
method starts with the true value  $u=\hat u$ and $l=\hat l$ are known; whereas the third method
maximizes the log-likelihood on the subset $y_i$ such that $l<y_i<u$.
The first and the second methods  method stalls for $u$ and $l$ yet $\theta$ is correctly found.
However in terms of cdf, there is no difference but the gof statistics do show differences.
The third method is rather bad.
```{r, fig.height=4, fig.width=6}
f1 <- fitdist(x, "texp", method="mle", start=list(rate=1/mean(x), low=0, upp=20))
f2 <- fitdist(x, "texp", method="mle", start=list(rate=1/mean(x), low=min(x), upp=max(x)), low=0)
...
legend("bottomright", lty=1:4, col=c("red", "green", "blue", "darkorange"), 
        leg=c("full MLE", "MLE fixed arg", "MLE on subset", "true CDF"))
cbind(coef(f1), coef(f2))
gofstat(list(f1,f2))
```
