...
For a good introduction, please read books such as Nocedal \& Wright (2006) 
or Bonnans, Gilbert, Lemarechal \& Sagastizabal (2006). We consider
...
2. order points such that $f(x_1)\leq f(x_2)\leq\dots\leq f(x_n)$.
...
4. Reflection: 
  ...
  + \textbf{if} $f(x_1)\leq f(x_r)<f(x_n)$, 
  then replace $x_{n+1}$ by $x_r$,
  go to step 2.

  + \textbf{else} go step 5.

5. Expansion:   
  
  + \textbf{if} $f(x_r)<f(x_1)$, then compute the expansion point
    $x_e= x_o+\gamma(x_o-x_{n+1})$.
   
   + \textbf{if} $f(x_e) <f(x_r)$, then replace $x_{n+1}$ by $x_e$,
  go to step 2.
  
   + \textbf{else} $x_{n+1}$ by $x_r$, go to step 2.
   
  + \textbf{else} go to step 6.

6. Contraction:  
  
  + compute the contracted point $x_c = x_o + \beta(x_o-x_{n+1})$
  
  + \textbf{if} $f(x_c)<f(x_{n+1})$, 
  then replace $x_{n+1}$ by $x_c$,
  go to step 2.
  
  + \textbf{else} go step 7.
...
* $\beta_k = \frac{ g_k^T (g_k-g_{k-1} )}{g_{k-1}^T g_{k-1}}$: Polak-Ribi\`ere update.
...
* \textbf{Repeat} until Armijo satisfied,
...
(4) the log-transformed parametrization with the (true) gradient.
...
Based on the two previous example, we observe that all methods converge to the same
point. This is rassuring.  
However, the number of function evaluation (and the gradient evaluation) is
