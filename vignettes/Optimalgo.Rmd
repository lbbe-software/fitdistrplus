...
Using `llsurface`, we plot the log-likehood surface around the true value (green) and the fitted parameters (red).
```{r, fig.width=5, fig.height=5}
llsurface(min.arg=c(0.1, 0.1), max.arg=c(10, 10), 
          plot.arg=c("shape1", "shape2"), fix.arg=list(scale=10), nlev=25,
          plot.np=50, data=x, distr="burr")
points(resBFGS[1,"BFGS"], resBFGS[2,"BFGS"], pch="+", col="red")
points(3, 4, pch="x", col="green")
```

```{r, fig.width=5, fig.height=5}
llsurface(min.arg=c(0.1, 0.1), max.arg=c(20, 20), 
          plot.arg=c("shape1", "scale"), fix.arg=list(shape2=3), nlev=25,
          plot.np=50, data=x, distr="burr")
