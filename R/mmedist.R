	...
	optim.method="default", lower=-Inf, upper=Inf, custom.optim=NULL, ...) 
{
	...
	{
		distname <- distr
	}else if (is.function(distr))
	{
		distname <- substring(as.character(match.call()$distr),2)
	}else
	{
		stop("the distr argument must be either a character or a function")
	}
	...
	if(meth != "closed formula")
	{
		if (is.function(distr))
			mdistname <- distr
		else if (!exists(mdistname, mode="function"))
...
#			names(estimate) <- c("location", "scale")
	...
	}else #an optimimisation has to be done
	{
		...
		#Cramer	- von Mises
		dCvM <- function(par, fix.arg, order, obs, mdistnam, memp)
		{
			momtheo <- do.call(mdistnam, c(as.list(order), as.list(par), as.list(fix.arg)) )
			momemp <- as.numeric(memp(obs, order=order))
			(momemp - momtheo)^2
		}
		fnobj <- function(par, fix.arg, obs, mdistnam, memp)
			sum( sapply(order, function(o) dCvM(par, fix.arg, o, obs, mdistnam, memp)) )
