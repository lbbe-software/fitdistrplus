                   ...
                   ynoise = TRUE, plotstyle = "graphics", ...)
{
  ...
  if(plotstyle == "graphics") 
  {
    ...
    return(invisible(list(obs = obsp, probabilities = fittedprob)))
  ...
  } else 
  {
    ...
    ggppcomp <-
      ggplot2::ggplot(data = fittedprob, ggplot2::aes(.data$values, .data$obsp, group = .data$ind, colour = .data$ind, shape = .data$ind, linewidth = .data$ind)) +
      ggplot2::xlab(xlab) +
      ggplot2::ylab(ylab) +
