...
Here you will find some easy examples of use of the functions of the library \pkg{fitdistrplus}. 
...
For details, see the documentation of each function, using the R help command (ex.: \code{?fitdist}). 
Do not forget to load the library using the function \code{library} before testing following examples.
<<>>=
...
First of all, the observed distribution may be plotted using the function \code{plotdist}.
%%% R code
<<fig=TRUE, eps=FALSE>>=
...
Descriptive parameters of the empirical distribution may be computed using the function \code{descdist}.
...
the data set may be boostrapped by fixing the argument \code{boot} to an integer above 10 in \code{descdist}. 
\code{boot} values of 
...
function \code{fitdist}. This function uses the maximum likelihood
method if the argument \code{method="mle"} (or if it is omitted) 
or the matching moments method if the argument \code{method="mom"}.
When fitting continuous\footnote{The reference book on continuous distribution
is \cite{kotz}.} distributions, 
...
also computed when possible. For this calculation, cells are defined by the argument \code{chisqbreaks}
or automatically defined from the data set and from the argument \code{meancount} (the approximate mean count per cell)
which is  fixed to $(4n)^{2/5}$ if omitted (with n the length of the data set).  
For more details, see the help of the function \code{fitdist}.
...
%%% R code
<<>>=
f1gbis<-fitdist(x1,"gamma",method="mom")
summary(f1gbis)
@ 
%%%
...
The values of the Anderson-Darling statistic (or another result of the fit: see the help of \code{fitdist} for details)
...
the parameters in \code{start} must correspond exactly to their definition in R or to their 
definition in a previous R code.
The function \code{plotdist} may help to find correct initial values for 
...
boostrap using the function \code{boodist}. This function returns the boostrapped values of parameters which
...
of the boostrap values of each parameter (see the help of the function \code{bootdist} for details).
...
may also be provided, fixing the argument discrete of the function \code{plotdist} to \code{TRUE}.
...
may be computed using the function \code{descdist} which also provides a skewness-kurtosis plot 
...
First of all, the observed distribution may be plotted using the function \code{plotdistcens}.
Data are reported directly as segments for interval, left and right censored data, 
and as points for non-censored data. For more details, see the help of the function \code{plotdistcens}.
...
left NA-values of positive data), the arguments \code{leftNA} (or \code{rightNA})
...
It is also possible to fix \code{rightNA} or \code{leftNA} to a realistic 
...
function \code{fitdistcens}. This function always uses the maximum likelihood
method.  For more details, see the help of the function \code{fitdistcens}.
...
boostrap only, using the function \code{boodistcens}.
...
As with \code{fitdist}, for some distributions (see the help of \code{fitdistcens} for details), 
...
the parameters in \code{start} must correspond exactly to their definition in R or to their 
definition in a previous R code.
The function \code{plotdistcens} may help to find correct initial values for 
