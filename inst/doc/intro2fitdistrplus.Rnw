...
This argument may be fixed to \texttt{"Nelder-Mead"} (the robust Nelder and Mead method), 
\texttt{"BFGS"} (the BFGS quasi-Newton method), \texttt{"CG"} (a conjugate gradients method), 
\texttt{"SANN"} (a variant of simulated annealing) or \texttt{"L-BFGS-B"} (a modification of the BFGS 
...
\section{Alternative methods to maximum likelihhod for non censored data}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
to \texttt{"mme"} in the call to fitdist (see the help file page of the function
...
to \texttt{"qme"} in the call to fitdist and adding an argument \texttt{probs}
...
%%%%%%%%%%
Parameters may also be estimated by maximizing goodness-of-fit (or minimizing a goodness-of-fit distance)
by fixing the argument \texttt{method}
to \texttt{"mge"} in the call to fitdist and adding an argument \texttt{gof}
coding for the goodness-of-fit distance chosen. One may use the classical 
Cramer-von Mises distance (\texttt{"CvM"}), the classical
Kolmogorov-Smirnov distance (\texttt{"KS"}), the classical Anderson-Darling distance (\texttt{"AD"})
