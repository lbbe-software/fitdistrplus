...
% A changer en cas de mise a jour de fitdistrplus !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
This manuscript reviews the various features of version 0.3-4 of \texttt{fitdistrplus}. The package
...
% TO DO : change the formalism of this equation
...
Weibull, lognormal and gamma distributions to ``groundbeef'' data set (Figure~\ref{compcdf}).
<<echo=TRUE, fig=FALSE>>=
...
% Ajouter ex endosulfan avec la bibliothËque actuar !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
% mais comment faire dans la vignette : indiquer que fitdistrplus nÈcessite actuar????????????????????????????
...
empirical cdf curve with confidence intervals, by calls to functions \texttt{survfit}
and \texttt{plot.survfit} from the \texttt{survival} package.
...
%%%%%%%%%%
Only one goodness-of-fit plot is provided for censored data (Figure~\ref{plotfitdistcens}), corresponding to the theoretical 
...
Moreover, the use of the whole bootstrap sample is of great interest in the risk assessment field. Its use enables
