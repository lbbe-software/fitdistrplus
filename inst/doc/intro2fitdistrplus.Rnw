...
Kolmogorov-Smirnov and Anderson-Darling statistics may be computed using the function \texttt{gofstat}. 
...
In somme cases, especially on very big datasets, even if the null hypothesis is rejected a fitted distribution may be 
...
Regarding the Kolmogorov-Smirnov statistic, the fit of a normal distribution seems better while regarding the 
